...
export async function loadSeed(read) {
	...
	const terrains = parseCSV(await read("terrains.csv")).map(r => ({ qid: r.qid, category: r.category, name_en: r.name_en, ne_extra: (r.ne_extra || "").split("|").filter(Boolean), axis: r.axis || "" }));   // 地形（ne_extra＝形状を結合する Natural Earth 側の別 QID・長江の金沙江など）（山脈・半島・砂漠・平原・海嶺・海溝・島・諸島）＝QID 基軸
