...
export const SEED_FILES = ["nations.csv", "cities.csv", "terrains.csv", "conflicts.csv", "overrides.json", "capital-notes.json", "aliases.json", "ja.json"];
export async function loadSeed(read) {
	...
	const conflicts = parseCSV(await read("conflicts.csv")).map(r => ({ key: r.key, qid: r.qid, type: r.type, region: +r.region, name_en: r.name_en, exist: r.exist === "true", sovereignt: r.sovereignt, territory: r.territory, claim: (r.claim || "").split("|").filter(Boolean) }));   // 係争地＝key 基軸（claim は | 区切り＝主張国の key／exist=true が実効支配のある実体）
	const [overrides, capitalNotes, aliases, ja, ui] = await Promise.all([J("overrides.json"), J("capital-notes.json"), J("aliases.json"), J("ja.json"), J("../i18n/ui.json")]);
