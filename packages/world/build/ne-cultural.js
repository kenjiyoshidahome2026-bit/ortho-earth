// Natural Earth Cultural（10m・版固定）を world の key ごとに切り分ける中核。Node（scripts/ne-cultural.mjs）と
// ブラウザ（apps/uploader の「国別DB (world)」節＝生成して bucket GIS/world/ へ置く）で共用＝依存は polygon-clipping と geopbf だけ。
// 取得（env.ne）と書き出し（gzip・ファイル/bucket）は呼び出し側の役。国の領域の定義は createGeometryPNG と同じ:
...
//     独立に切り出す＝主の分割と重複する（world の geo_tub と同じ）。その型のポリゴンが admin_0 層
// 線（鉄道・道路）は境界で切り、面（市街地・湖）は境界を跨ぐものだけ polygon-clipping で交差を取る。この 3 層は属性を持たず key ごとに
// 1 地物（MultiLineString / MultiPolygon）に束ねる。海上の断片（フェリー丸ごと＋領域の外を通る区間・1km 未満の外れは繋ぐ）は
// 国のデータに入れず routes として別に持つ＝航路。点は内外判定（沿岸で外れた点は最寄りの領域へ）。admin1 は属性で束ねる。
...
// seed＝build/seed.js の loadSeed の結果（nations / conflicts / overrides を使う）。
// env＝{ ne(name)→GeoJSON features（取得とキャッシュは呼び出し側）, log, warn }。
// opts＝{ precision=6, layers=NE_LAYERS, only=Set|null }。
// 戻り＝{ all（1 層に平らにした全地物）, out（key → layer → features）, routes, index（要約・single は呼び出し側が足す）, encode(name, features, description)→ArrayBuffer }
export async function buildNeCultural(seed, env, opts = {}) {
	const log = env.log || console.log;
	const PRECISION = opts.precision ?? 6;
	const LAYERS = new Set(opts.layers || NE_LAYERS);
	const ONLY = opts.only ? new Set(opts.only) : null;
	const ne = env.ne;
	const nations = seed.nations.map(n => ({ key: n.key, qid: n.qid, name: n.name_en, territory: n.territory, conflict: n.conflict }));
	...
	const conflicts = seed.conflicts, overrides = seed.overrides;
	...
	const encode = async (name, features, description) => (await new GeoPBF({ name, precision: PRECISION, attribution: ATTR, description }).set({ type: "FeatureCollection", name, features })).arrayBuffer;
	return { all, out, routes, index, encode, countVerts };
}
