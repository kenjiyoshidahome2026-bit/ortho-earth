// 言語別テーブル i18n/<lang>.json＝{ updated, lang, rtl, nations:{key:{name,wiki[,official,yomi]}}, cities:{qid:…}, terrains:{qid:…}, languages:{key:…}, currencies:{key:…}, conflicts:…, ui, categories, plateBoundaries }{key:…}, ui:{…} }
...
export function buildI18N(seed, { NE, CE, LC, KE, TE }, { NationDB, CityDB, TerrainDB, LanguageDB, CurrencyDB, Conflicts }) {
	...
	for (const lang of seed.langs) {
		...
		const jaN = lang == "ja" ? seed.ja.nations : {}, jaC = lang == "ja" ? seed.ja.cities : {}, jaT = lang == "ja" ? (seed.ja.terrains || {}) : {};
		out[lang] = {
			...
			terrains: tbl(TerrainDB, TE, t => t.qid, t => jaT[t.qid]),   // 日本語ラベルの無い地形は seed/ja.json の terrains で補う
			...
			categories: Object.fromEntries(Object.entries(seed.ui.categories || {}).map(([k, v]) => [k, v[lang]]).filter(([, v]) => v)),   // 地形の分類名（Wikidata のクラスのラベル・scripts/i18n-classes.mjs）
			plateBoundaries: Object.fromEntries(Object.entries(seed.ui.plateBoundaries || {}).map(([k, v]) => [k, v[lang]]).filter(([, v]) => v)),
		};
	}
	return out;
}
