async function createCityDB(toLangs) {
	const yomi_array = [
	...
	async function getCities() {
		const html = await d3.wiki.getContent("100万都市の一覧");
	...
	async function cleanCities(cities, toLangs) {
		...
		var ids = await d3.wiki.title2id(titles);
		ids.forEach((t, i) => cities[i].wiki = { ja: t });
		var capitals = [];
		var bigcities = [];
		nations.forEach(nation => {
			if (!nation.capital) return;
			const name = nation.name.ja, wiki = nation.capital.wiki.ja;
			const city = cities.filter(t => t.nation == name);
			const ids = city.map(t => t.wiki.ja);
			const v = { name: nation.capital.name, wiki: nation.capital.wiki, nation: name, capital: true }
			capitals.push(v);
			city.forEach(t => t.wiki.ja == wiki ? (v.population = t.population) : bigcities.push(t));
		});
		////----------------------------------- 重複する都市をまとめる
		...
		////----------------------------------- 各言語の名称及びwikiIDの生成
		var titles = bigcities.map(t => t.title);
		const wikija = (await d3.wiki.title2id(titles));
	...
	async function getCoords(cities) {
		var coords = await d3.wiki.id2coords(cities.map(t => t.wiki.en), "en");
		await d3.thenEach(cities, async (t, i) => {
			if (coords[i]) {
	...
	async function population(cities) {
		...
		await d3.thenEach(cities, async t => {
			const html = await d3.wiki.getContent(t.wiki.ja);
			...
			if (!Array.isArray(t.population)) {
				console.warn("no population data", t);
				t.population = [0, 0];
			}
		});
		function 年(html) {
