...
// packages/geopbf/src/extension/topology.js

import { 
	init_shared_buffer,
    pack_gint_morton,
    get_wasm_memory, // 🚀 追加
    bulk_l1_to_l2_wasm, // 🌟 傑作ロジックを保持
    allocate_u8_buffer,
    free_u8_buffer,
    allocate_u32_buffer,
    free_u32_buffer,
...
// 🌟 【最終正解】第2引数で、Worker側から初期化済みの `wasm` オブジェクトを正規ルートで受け取る！
export function topology(self, wasm) {
	const wasmMemory = get_wasm_memory();
    const sharedGintPtr = init_shared_buffer();
    const sharedGintBuffer = new BigUint64Array(wasmMemory.buffer, sharedGintPtr, 524288);
    ...
    self.each((i, map) => {
        ...
        const process = (pos, type) => {
            ...
            let end = 0;
                    ...
                    // 🚀 吉田さんのオリジナルの read ロジックを 100% 完全維持！
			 const read = (n) => {
				let count = 0, x = 0, y = 0;
				const grab = () => { elemCount[3]++;
					...
					if (dx || dy) { 
						x += dx; y += dy; 
						// 🌟 共通バッファへ最速モートン結果をダイレクト上書き代入！
						sharedGintBuffer[count++] = pack_gint_morton(x * S | 0, y * S | 0); 
					}
				};
				if (n === undefined) { while (pbf.pos < end) grab(); }
				else { while (n-- > 0) grab(); }
				
				// 🌟 今回書き込んだ要素数分（0 ～ count）だけを一撃で最速スライスコピーして返却
				return sharedGintBuffer.slice(0, count);
			};
            pbf.readMessage((tag) => {
            ...
            if (type === 4 || type === 3 || type === 5) {
                elemCount[tIndex]++;
                structures[tIndex].push({ id, coords: coords });
            } else {
                coords.forEach(c => {
                    elemCount[tIndex]++;
                    structures[tIndex].push({ id, coords: c });
                });
            }
        };
    ...
    // structuresから合計頂点数を一撃で逆算
    let totalPoints = 0;
    structures[0].forEach(f => { if (f.coords) f.coords.forEach(function check(item) { if (item instanceof BigUint64Array) totalPoints += item.length; else if (Array.isArray(item)) item.forEach(check); }); });
    structures[1].forEach(f => { if (f.coords) f.coords.forEach(function check(item) { if (item instanceof BigUint64Array) totalPoints += item.length; else if (Array.isArray(item)) item.forEach(check); }); });
    structures[2].forEach(f => { if (f.coords instanceof BigUint64Array) totalPoints += f.coords.length; else if (Array.isArray(f.coords)) f.coords.forEach(item => { if (item instanceof BigUint64Array) totalPoints += item.length; }); });
    elemCount[3] = totalPoints;
    ...
    structures[0].forEach(({ id, arcs }) => { 
        arcs.forEach(t => { t.forEach(arc => { arc = arc < 0 ? ~arc : arc; owner[arc] = owner[arc] || []; owner[arc].push(id); }); });
    });
    owner.filter(ids => ids.length > 1).forEach(ids => {
         ids.forEach(id => { neighborTub[id] = neighborTub[id] || new Set(); ids.filter(t => t !== id).forEach(t => neighborTub[id].add(t)); });
    });
    ...
    const topologyStr = JSON.stringify([polygonTub, polylineTub, neighborTub]);
    const bbox = initBbox();
    ...
    const arcCount = (polygon ? polygon.count : 0) + (polyline ? polyline.count : 0);
    ...
    const totalLength = headLength * 4 + arcLength * 8 + arcCount * 8 * 4 + pointCount * (8 + 4) + topologyStr.length;
    ...
    view.set(new Uint8Array(new TextEncoder().encode(topologyStr)), ptr), ptr += topologyStr.length;
    
    if (totalLength !== ptr) throw new Error(`GintBUF length mismatch: expected ${totalLength}, got ${ptr}`);
    
    //free_u8_buffer(pbfPtr, pbfLen);
    return GintBUF;
}
...
function buildArcs(buffs, metas) { // 複数Arcの一括パルス化
    ...
    gint.L1toL2_js(buffer, meta, count, mlen);
    console.log(`🟢 [JS Engine] Complete in ${(performance.now() - t0).toFixed(2)} ms`);
