...
onmessage = async (e) => {
	const { file, name, precision, description, license, attribution, layer, ignoreCrs } = e.data;
	try {
		const entries = await decodeZIP(file);
		if (!entries) throw new Error("zip を開けない");
		const source = gdbSourceFromFiles(entries);
		const { pbf, stats } = await fromFileGDB(source, { name: name && !/\.gdb$/i.test(name) ? undefined : undefined, precision, description, license, attribution, layer, ignoreCrs });
