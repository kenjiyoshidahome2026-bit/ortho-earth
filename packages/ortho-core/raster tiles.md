...
## 8. 地面アトラス（RTT ドレープ）への統合（2026-09-21・裁定＝3D だけ／3 段窓で全部アトラス／3D のフェードは省く）

- 頂点で貼る方式（塗り VS 流用）は「地形の折れ目をまたぐ弦が地形に潜る」＝細分・深度バイアス・接地リフトでも尾根沿いに残った（iPhone 実機）。
  本人「基図をしっかり合成してから処理」→ **ラスタは先にアトラスへ合成し FS が画素標本化**（PR#21）→ さらに **ベクタ塗りも 3D では同じアトラスへ焼く**（本節）。
- `src/ground.js`：3 段窓（近＝視野中心 ±1 画面幅・zoom 1/4 段量子化／中 6 倍／遠 36 倍・原点は半幅の 1/4 刻みにスナップ）＝両バックエンド共通の純関数。
- 合成順＝ラスタ基図（under）→ ベクタ塗り（3D 時・base→main・海の点火ゲート・図郭外水域ゲート＝標高テクスチャ・3D の足元塗りは伏せる）→ ラスタ重ね（over）。
  鍵（窓・ラスタ rev・シーン rev・ゲート・baseAlpha・slots）が変わった時だけ再合成＝静止中ゼロコスト。GL2＝FBO×3＋`ATLAS_FILL_VS/FS`（属性位置は
  塗り本体の VAO に合わせて bindAttribLocation・multi_draw 変種は mdize）、WebGPU＝`ATLAS_FILL_WGSL`（group0=Frame/標高・group2=AtlP dynamic offset）・
  合成パス用の bind group はアトラス自身を dummy にする（描き込み先を同期スコープ内で読むと CommandBuffer ごと無効）。
- 標本化＝`gndMix`（GL）／`gndMix0`（WGSL）：3 枚を無条件に標本化してから近→中→遠の順で選ぶ（分岐内の texture() を避ける）。TERRAIN_FS・GLOBE_FS（床）・FILL_FS（2D のラスタ重ね）。
- 3D では塗りの直描きを止める（`rasterHide = gnd.fillsIn || …`）＝水面リフト +30/+10m・都市帯 +5m・水域の厳密対数深度は塗りには不要に（線は従来のまま）。
  2D（真俯瞰・地形なし）は直描きのまま＝平面図の鮮鋭さは不変。
