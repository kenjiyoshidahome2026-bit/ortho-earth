...
- **fid 別 bbox は周期対応（2026-09-15 夜）**：bake の buildPolyBboxByFid は縫い目を跨ぐ feature（切断で片が両側）の arc bbox を
  周期で近い側へ合流し、x が 360e7 を超える表現（例 [358e7, 362e7]）を許す。消費側は周期で読む＝pivotClip/capVisible は中心を
  mod 360e7、bboxVisible と geopbf findPolygon は「東片 [0, x1−360e7] と西片 [x0, 360e7] の 2 区間」で交差判定。fit の重心も正しく
  180° 近傍へ。u32 に収まらない（西片が縫い目から 69° 超）時だけ従来の全幅＝下の全幅フォールバックが効く。
  発端＝全幅 bbox のキャップ免除で、反対側から見た縫い目跨ぎの円（対蹠点を囲む）が地平円一周＝円盤全面塗り（本人報告）。
- **縫い目跨ぎ feature（bbox 経度全幅）の扱い（2026-09-15）**：扇の要（pivotClip）は bbox 中心＝経度 0°＝地球の裏側になるため、
  裏側の要から張る扇が遠クリップ面で切られて「要の投影位置を中心とした環の大きさ比例の円盤」が塗り残る（geoedit の縫い目跨ぎの円で
  「自分自身の影」）。bbox 経度スパン ≥180° の feature は要をクリップ原点にし、球面キャップ判定（capVisible）も免除。
  線パスは両端が縫い目上（ix=0 or 360e7）の辺＝切断で生えた縦辺を描かない（塗りの巻き数には残す）。
- **球体の扱い（塗り扇の共通契約・2026-09-15）**：扇の端点は VS で `horizonClamp`＝裏半球（dot(P,E)<1）の頂点を
  地平円（中心 E/|E|²・半径 √(1−1/|E|²)）へ射影クランプ（renderer STENCIL_VS `u_sphereClip` と同式）。裏の面は
  円周上に縮退＝巻き数 0、地平線を跨ぐ面は可視部だけを囲む。加えて `bboxVisible` は視野 bbox の後に
  **球面キャップ判定**（bbox を囲む小円 (C,rad) が可視キャップ (Ê,hor) と交わらなければ棄却・rad は矩形の最遠点を
  解析的に＋3.5° 余裕）＝対蹠点を囲む面が「環が地平円を一周＝全面 ±1」になるのを防ぐ。境界メタ（fid 混成ループ・
  bbox テクスチャ無し）はキャップ判定なし＝クランプのみ。無しだと球を透かした投影の折返しが「裏の面のゴースト」と
  「跨ぎ面の表側が塗れない（±1 相殺）」を作る（geoedit で顕在化・回帰＝tests/t-backfill.html GL/WebGPU 両方）
