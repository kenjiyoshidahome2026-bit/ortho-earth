...
function move(data) { const q = accessories.latlng, names = q.names;
...
function draw_latlng() { const q = accessories.latlng; leave()
...
function draw_scale() { const q = accessories.scale;
...
function draw_credit() { const q = accessories.credit;
...
function draw_globe() { const q = accessories.globe;
...
function draw_night() { const q = accessories.night;
	if (!q || zoom > q.maxZoom) return;
	const dt = new Date();
	const cx = width / 2, cy = height / 2, er = proj.scale(), er2 = er * er;	
