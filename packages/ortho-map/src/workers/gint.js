...
function drawing(data) {
	...
	const lodThresh = Math.max(0, 63 - (zoom - SWITCH_ZOOM) * 10);
...
function initWebGL(gl, dpr) {
	...
	gl.resizeBySize = (w, h) => {
		...
		gl.useProgram(program);
		gl.uniform2f(gl.viewport_loc, canvas.width, canvas.height);
...
function updateJacobian(width, height) {
	...
	const centerIntX = Math.round((centerLng + 180) * 1e7);
	const centerIntY = Math.round((centerLat + 90) * 1e7);
