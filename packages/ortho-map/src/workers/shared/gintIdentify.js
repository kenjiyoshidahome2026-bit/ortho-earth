...
export function doIdentify(data) {
	...
	const pickX = Math.max(0, Math.min(Math.round(s.width  * s.dpr) - 1, Math.round(data.x * s.dpr)));
	const pickY = Math.max(0, Math.min(Math.round(s.height * s.dpr) - 1, Math.round((s.height - data.y) * s.dpr)));
...
export function handleMove(data) {
	if (!s.lastProj || !s.gintData || s._isDrawing) {
