...
export const Resources = async(flag = true) => {
    const resources = {
        icons: gadgetIcons,
    ...
    if (flag === false) return resources;
    resources.sphere = resources.sphere || { type: "Sphere" };
    resources.graticule = resources.graticule || d3.geoGraticule10();
    resources.border = resources.border || (await geopbf("ne_50m_admin_0_boundary_lines_land")).geojson;
    resources.maritime = resources.maritime || (await geopbf("ne_50m_admin_0_boundary_lines_maritime_indicator")).geojson;
    resources.lines = resources.lines || (await geopbf("ne_50m_geographic_lines")).geojson;
    resources.land = resources.land || (await geopbf("ne_50m_land")).geojson;
    resources.land110 = resources.land110 || (await geopbf("ne_110m_land")).geojson;
    resources.stars = resources.stars || (await geopbf("stars.8")).geojson;
    return resources;
}
