            ...
            if (tag === TAGS.LENGTH) pbf.readPackedVarint(lens);
                ...
                const lineTo = () => {
                    ...
                    if (first) {
                        ...
                        if (prevP[2] >= 0 && currP[2] >= 0) {
                            // 🌟 両方手前：普通に線を引く
                            ctx.lineTo(currP[0], currP[1]);
                        } else if (prevP[2] < 0 && currP[2] < 0) {
                            // 🌟 両方裏側：何もしない！（無駄なmoveToを排除）
                        } else {
                            // 🌟 地平線を跨ぐ場合：ユーザー実装の完璧な交点計算！
                ...
                if (type === 0 || type === 1) { // Point系
                    while (pbf.pos < end) {
                        const currP = readNext();
                        if (currP[2] >= 0) {
                            const radius = (properties.size || 6) / 2;
                            ctx.moveTo(currP[0] + radius, currP[1]);
                            ctx.arc(currP[0], currP[1], radius, 0, PI * 2);
                        }
                    }
