// 地球儀のホスト（packages/globe）とその殻（apps/*）の UI 文言を「英語キーの集合」として読み出す唯一の走査器（i18n-extract と verify-i18n の共通の目）。
// 住所＝globe（本体の訳の持ち主）。殻は APP を渡して使う（2026-09-24 に apps/ortho-japan/scripts/lib から移設＝ortho-globe と共有）。
//
...
// LITERAL_ROOTS＝アプリの外にある、t() は呼ばないが英語キーをデータとして持つ所（ortho-core＝worldstyle のテーマ名札 "Blank map" 等・段階 3）
// ＝文字列だけを「在る」印に数える（無いと移設したキーが次の i18n:extract で退役する・2026-09-22）。opts.only（ページ別の走査）には足さない。
// 地域パック（japan なら @ortho-earth/jp の出典・画像タイルの key）は殻が i18n/pages.json の literalRoots で足す＝globe は地域名を書かない。
export const LITERAL_ROOTS = ["../../packages/ortho-core/src"];
...
export function scanApp(appDir, ctx = {}, { exclude = null, only = null, extraRoots = [{ dir: hostDir(appDir), prefix: HOST_PREFIX }], literalRoots = LITERAL_ROOTS } = {}) {
	...
	if (!only) for (const rel of literalRoots) {
