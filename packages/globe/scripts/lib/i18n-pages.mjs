...
import { scanApp, LITERAL_ROOTS } from "./i18n-scan.mjs";
export function loadPages(APP) {
	...
	return { pages, fileToPage, tables, pageFiles: new Set(fileToPage.keys()), literalRoots: cfg.literalRoots ?? LITERAL_ROOTS };   // literalRoots＝殻の申告（地域パック等）
}
...
export function scanAll(APP, ctx) {
	...
	const r = scanApp(APP, ctx, { exclude: P.pageFiles, literalRoots: P.literalRoots });
