// PLATEAU バッチデコーダ（②区内デコード並列化・2026-08-28）：meshworker が発注する「バッチ丸ごと」
//（fetch→b3dm/Draco解凍→ECEF→世界座標→dedup→接地→LOD→RTE→マスク断片）を別コアで実行して返すだけの器。
// 実体は全て meshdecode.js＝区workerの直列経路と同一関数（経路差ゼロ）。区単位の状態（wardMask累積・
...
import { decodeBatch, setDecodeEnv } from "./meshdecode.js";
...
const handle = async d => {
	...
	catch (err) { console.warn("[mesh] pooled batch failed", d.base, err?.message ?? err); }
