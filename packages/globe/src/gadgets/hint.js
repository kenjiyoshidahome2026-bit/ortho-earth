...
export function hint({ signal, onOpen } = {}) {   // onOpen＝カードを開いた瞬間の合図（globe の「道具の排他」＝計測・日影などを閉じる）
	...
	card.innerHTML = coarse ? `
		<button id="hint-close" title="${t("Close")}" aria-label="${t("Close")}">×</button>
		<div class="hint-title">${t("Controls")}</div>
		<div class="hint-row"><span class="key">${t("1 finger")}</span><span>${t("drag = <b>pan</b>")}</span></div>
		<div class="hint-row"><span class="key">${t("2 fingers")}</span><span>${t("spread / pinch = <b>zoom</b>")}</span></div>
		<div class="hint-row"><span class="key">${t("2 fingers")}</span><span>${t("twist = <b>rotate</b>")}</span></div>
		<div class="hint-row"><span class="key">${t("2 fingers")}</span><span>${t("pull up / down = <b>3D view</b> (tilt)")}</span></div>
		<div class="hint-note">${t("Zoom in to a city and buildings appear in 3D<br>(the first load takes a moment)")}</div>` : `
		<button id="hint-close" title="${t("Close")}" aria-label="${t("Close")}">×</button>
		<div class="hint-title">${t("Controls")}</div>
		<div class="hint-row">${MI_L}<span>${t("drag = <b>pan</b>")}</span>${MI_W(" gap")}<span>${t("wheel = <b>zoom</b>")}</span></div>
		<div class="hint-row"><span class="key">⌘/Ctrl</span><span class="hint-plus">＋</span>${MI_W("")}<span>${t("= <b>rotate</b>")}</span></div>
		<div class="hint-row"><span class="key">⇧/Shift</span><span class="hint-plus">＋</span>${MI_L}<span class="hint-plus">／</span>${MI_R}<span>${t("right drag")}</span></div>
		<div class="hint-row"><span>${t("drag = <b>3D view</b> (tilt / rotate)")}</span></div>
		<div class="hint-note">${t("Zoom in to a city and buildings appear in 3D<br>(the first load takes a moment)")}</div>`;
	const btn = document.createElement("button");
	btn.id = "hint-btn"; btn.dataset.tip = t("Controls (?)"); btn.textContent = "?"; btn.setAttribute("aria-label", t("Controls"));
