...
function 節月計算(year, tdiff = 9) {
	let angle = []; 
	for (let i = 0; i < 12; i++) angle.push(r360(285 + i * 30));
	const dt = initAngle(year, tdiff);
	angle = angle.map(r => Math.floor(SunDegreeDay(year, r) - dt) - 1);
	angle.unshift(Math.floor(SunDegreeDay(year - 1, 255) - dt) - 1 - yearLength(year - 1));
	return angle;
}
function 節日計算(YMD) {
...
export function 日カレンダー(年月日) { 
	...
	<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" xlinkns="http://www.w3.org/1999/xlink" version="1.1" style="user-select: none;">
		...
		<g fill="#444" dominant-baseline="middle" text-anchor="middle" font-family="Arial">
			...
			<text transform="translate(77,${special?73:70})" font-size="3">${日数}</text>
			<g transform="translate(85,65) scale(0.1)">${月相(旧暦.日 - 1)}</g>
...
function 月カレンダー(YMD) {
	...
	for (let i = 1; i <= monthLength(YMD); i++) { 
		const x = w * (n + 1);
