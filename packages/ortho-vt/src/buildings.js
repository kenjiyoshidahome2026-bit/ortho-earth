...
export function buildBuildings({ layers, z, x, y }, origin) {
	...
	for (const f of src.features) {
		...
		for (const [flat, holes] of polygons(f.geom)) {
			...
			// 屋根（上面, height=h）
			for (const idx of earcut(flat, holes, 2)) { const p = ll[idx]; pos.push(p[0], p[1], h); shade.push(ROOF); }
			...
			for (let r = 0; r < ringStarts.length - 1; r++) {
				...
				for (let i = a; i < b; i++) {
					...
					pos.push(p0[0], p0[1], 0, p1[0], p1[1], 0, p1[0], p1[1], h); shade.push(WALL, WALL, WALL);
					pos.push(p0[0], p0[1], 0, p1[0], p1[1], h, p0[0], p0[1], h); shade.push(WALL, WALL, WALL);
				}
			}
		}
	}
	return pos.length ? { pos: new Float32Array(pos), shade: new Float32Array(shade) } : null;
}
