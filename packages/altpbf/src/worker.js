...
import { decode, encodeName } from "./altpbf.js";
...
onmessage = async e => { 
    ...
    const { name, lng, lat, range } = e.data;
    try { 
        ...
        if (!v) return postMessage({ error: `not found: ${name}`, name });
        success(await decode(v));
        await cache(new File([v], name, {type:"application/x-altpbf"}));
        function success(v) { 
            if (!v) return postMessage({ error: "No data", name });
            const transfer = (v.data && v.data.buffer) ? [v.data.buffer] : [];
            postMessage({ ...v, name }, transfer); 
        }
    } catch (err) { postMessage({ error: err.message, name }); }
};