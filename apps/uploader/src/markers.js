...
const size2WH = size => isArray(size)?size:[size,size];
...
////---------------------------------------------------------------
export async function fromIMG(file, size) {
	const [width,height] = size2WH(size);
	const bitmap = isURL(file)? await url2bitmap(file, w, h):
		isFile(file)?
		(file.type == "image/svg+xml")? await fromSVG(await file.text(), size):
		(file.type.match(/^image/))? await image2bitmap(file, w, h):null:null;
	if (!bitmap) console.error(`fail to create bitmap: ${file}`);
	const offsetX = -width / 2, offsetY = -height / 2;
	return { bitmap, width, height, offsetX, offsetY };
}
export async function fromSVG(name, size) { 
	const [width, height] = size2WH(size);
	await initialize();
	const file = await cache(name);
	const bitmap = file? await svg2bitmap(await file.text(), width, height):
	name.toString().match(/\<svg/)? await svg2bitmap(name, width, height): null;
	if (!bitmap) { console.error(`fail to create bitmap: ${name}`); return null; }
	const offsetX = -width / 2, offsetY = -height / 2;
	return {bitmap, width, height, offsetX, offsetY};
}
export async function marker(size = 32, fill="#800", stroke="#fff",label=true) {
	...
	const v = await fromSVG(svg, size); v.offsetY = -v.height;
	return v;
}
export async function paramSVG(name, size, n = 1, color = "#f00") {
	const [width, height] = size2WH(size);
	const svg = {
	...
	const bitmap = svg? await svg2bitmap(svg(n, color), width, height): null;
	if (!bitmap) { console.error(`fail to create bitmap: ${name}`); return null; }
	const offsetX = -width / 2, offsetY = -height / 2;
	return { bitmap, width, height, offsetX, offsetY };
}

// export function drawMarker(ctx, [x, y]) { const self = this, {bitmap, width, height, offsetX, offsetY} = self;
