...
import { APP, uiKeys, demoTextKeys } from "./keys.mjs";
...
const page = uiKeys(), demos = demoTextKeys().filter(k => !page.includes(k));
const errs = [], warns = [];
...
console.log(`page keys: ${page.length} · demo keys: ${demos.length} · ui.json: ${Object.keys(ui).length} · languages: ${langs.length}`);
