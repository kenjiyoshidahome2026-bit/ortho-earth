...
// 台帳は munic/manifest.json（munic-manifest.js が作る・アプリ同梱）。データ本体は配布元を直読みしてその場で GeoPBF 変換
// （G空間は CORS 可・取れない先は native-bucket の中継が自動で拾う＝bucket には置かない）。
...
const hasCad = e => e.sets.some(s => s.cat === CAD);
const pageOf = s => `https://www.geospatial.jp/ckan/dataset/${encodeURIComponent(s.id)}`;
...
function cityItemHtml(c) {
	...
	return `
		<div class="moj-city-item" data-code="${escHtml(c.code)}" title="開く">
			...
			<span class="moj-city-name">${escHtml(c.name)}${hasCad(e) ? '<span class="area-tag munic-cad-tag">地番図</span>' : ''}</span>
			<span class="moj-city-file">${e.sets.length} データセット</span>
			<span class="badge fmt-none">G空間</span>
		</div>`;
}

export function renderMunicList() {
	const src = cadOnly ? MUNIC.filter(hasCad) : MUNIC;
	const cities = src.map(e => ({ code: e.code, name: e.city, pref: e.code.slice(0, 2), _raw: e }));
	renderMinistryList({
		id: 'munic',
		title: '市区町村のオープンデータ',
		subtitle: `G空間情報センターに市区町村が公開しているデータ（${MUNIC.reduce((s, e) => s + e.sets.length, 0).toLocaleString()} データセット）
		...
		bulkByGroup: () => [],
		allEntries: () => [],
		onItemClick: c => renderCity(c._raw),
...
function toEntry(e, set, r) {
	return {
		...
		target: r.url,
		link: pageOf(set),
		attribution: `${e.orgTitle}（G空間情報センター）`,
		license: set.license,
		...
		<div class="detail-inner munic-city">
			...
			<header class="detail-header">
				...
				<div class="detail-meta">
					...
					<a class="ext-link" href="https://www.geospatial.jp/ckan/organization/${escHtml(e.org)}" target="_blank" rel="noopener">G空間情報センター「${escHtml(e.orgTitle)}」→</a>
					<span class="mono">${e.sets.length} データセット</span>
				</div>
				<p class="munic-note">資料を押すと配布元から読み込み、その場で GeoPBF に変換します。CSV・Excel は緯度・経度の列があるものだけ地図に描けます。</p>
			</header>
			${groups.map(([cat, sets]) => `
