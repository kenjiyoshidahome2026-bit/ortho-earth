...
// dlserveyId=A002005212015 => 2015年度国勢調査
// dlserveyId=A002005212020 => 2020年度国勢調査(R2)
const MAP_URL = (pref) => domain+"gis/statmap-search/data?dlserveyId=A002005212020&coordSys=1&format=shape&downloadType=5&code="+(pref);
////=====================================================================================
...
JADR.GEO = $.extend(JADR.GEO||{}, {
  ...
  世帯収入R2,
  民賃情報R2,
// ------------------------------（ここは最新だけで良いはず ... ?）=> 国勢調査の一部なので洗い替え！
  町丁目R2,
  // => 小地域（町丁・字等別）（JGD2000）
  // shape ファイルと関連しているため、常に最新を使う必要がある。
  // 結果、町丁目単位では旧データとのマッピングは取れない。
  地域名MAPR2,
  地域名11桁R2,
//  市区町村履歴:市区町村履歴,
//  役所住所:役所住所
});
$.extend(JADR.GEO.国勢調査R2, {
  人口:d=>表示(d,'人口'),
  世帯:d=>表示(d,'世帯'),
  住居:d=>表示(d,'住居'),
  産業:d=>表示(d,'産業'),
...
JADR.国勢調査情報R2 = {
  人口:code=>印刷(code,人口R2),
  世帯:code=>印刷(code,世帯R2),
  住居:code=>印刷(code,住居R2),
  産業:code=>印刷(code,産業R2),
  職業:code=>印刷(code,職業R2)
};
JADR.国勢調査表示R2 = {
  人口: 人口R2, 世帯: 世帯R2, 住居: 住居R2, 産業: 産業R2, 職業: 職業R2
};
// 国勢調査情報 [ UEtHux0U ] の上書き
// => const settings = (JADR.EXT[name]||{}).settings || {}, r_sets = settings.reportPrint || { loadObject: name, from: "1984" };

//JADRTOPでもREPLACEしている、町丁目shapeファイルから取得できるコードと、国勢調査（最新コード）との差異
const 町丁目データ = ['町丁目R2', '地域名11桁R2', '地域名MAPR2'];
const REPLACE = code => (code||'').replace(/^04423/,"04216").replace(/^40305/,"40231");//宮城県富谷市/福岡県那珂川市
const REPBAKS = { 'H27': code => (code||'').replace(/^40231/, "40305") };
////==============================領域=======================================================
function 表示(d, func) { const code = REPLACE(d.properties.id), name = d.properties.name||"", area = d.properties.area||JADR.geometryArea(d.geometry);
  let func_n; if(typeof func == 'string') { func_n = func, func = JADR.国勢調査表示R2[func_n]; }
  const GEO_org = JADR.GEO.国勢調査;
  const $g = GEO_org.$panel = $(`<div style="position:relative;width:480px;display:grid;grid-template-rows:30px 1fr;grid-template-columns:50% 50%;">
    <div style="grid-row:1;grid-column:1 / 3;text-align:center;margin-top:8px;" class="cmn">
      <h3><span name="地区名"/></h3>
      <div style="position:absolute;display:inline-block;top:0px;text-align:left;white-space:nowrap;" class="hikaku">
        <div name="総人口比較"><span>総人口: </span><span name="総人口増減" style="float:right;"></span></div>
        <div name="世帯数比較"><span>世帯数: </span><span name="世帯数増減" style="float:right;"></span></div>
      </div>
    </div>
    <div style="grid-row:2;grid-column:1;" class="cmp"> </div>
    <div style="grid-row:2;grid-column:2;" class="R02"> </div>
  </div>`);
  const max_w = window.innerWidth - 80, pan_w = 480;
  const div = $(ECDB.HTML("#base"));
  $g.find('.R02').append(div);
  // (2023.01.18 sakamoto) 2年分の国勢調査結果を並べて表示が可能なパネルとなる。
  // 国勢調査比較が OFF の時および、横幅が 480px を下回る時に比較は出力しない。
  // TODO controlBox のチェック状態を DOM から取得しない方法があれば変更
  // (2023.02.07 sakamoto) 町丁目単位の比較はコードの煩雑な変更により、不可能。
  // また、R2国勢調査は最大9桁までしかないので、そこでデータ取得する。
  // これも cmp_v の対象外とする。
  const cmp_v = !((code.length > 5) || (max_w < pan_w)) && $('.controlBox').find('input#_国勢調査比較').prop('checked');
  if(cmp_v) {
    $g.css('grid-template-columns', '50% 50%').css('width', `${pan_w}px`);
    $g.find('.hikaku').css('display', 'inline-block');
    $g.find('.cmp').css('display', '');
  } else {
    $g.css('grid-template-columns', '0 100%').css('width', `${pan_w/2}px`);
    $g.find('.hikaku').css('display', 'none');
    $g.find('.cmp').css('display', 'none');
  }
  GEO_org.code['R02'] = { code, name, prop: d };
  GEO_org.data['R02'] = null;
  return JADR.GEO.国勢調査R2("00").then(t=>JADR.GEO.国勢調査R2(code.substr(0, 9)).then(val=>{ if (!val&&!name) return div[0];
    GEO_org.data['R02'] = val;
    const areaName = code.length ==2? JADR.地域名称(code): code.length == 5? JADR.地域名称(code.substring(0,2))+JADR.地域名称(code): JADR.地域名称(code.substring(0,5))+name;
    if(!val||!func) {
      div.html(areaName);
      return div[0];
    }
    // $("[name=地区名]",div).text(areaName).css({fontSize:areaName.length > 12?"100%":"125%"});
    $g.find('.cmn [name=地区名]').html(areaName).css({ fontSize:areaName.length > 12?"100%":"125%", userSelect: 'text' });
    $("[name=地区名]",div).text('[ 令和2年/2020年 ]');
    ...
    func(div, val, t, code);
    $("[name=領域]", div).text(JADR.国勢調査関数.町丁目名最適(code, areaName));
    $("[name=市区町村]", div).text(JADR.地域名称(code.substring(0,5)));
    //
    //（以下、直接本年表示に関係のない追加処理）
    // (1) max-width の自動調整
    setTimeout(()=>$g.is(':visible') && $g.parent().css({ 'max-width': `${max_w}px`, 'width': 'auto' }), 80);
    // (2) 比較情報の表示
    if(cmp_v && func_n) 比較表示(d, func_n, $g);
    //
    return $g[0];
  }));
}
function 比較表示(d, func, $g) { const code = REPBAKS.H27(REPLACE(d.properties.id)), name = d.properties.name||"", area = d.properties.area||JADR.geometryArea(d.geometry);
  let func_n; if(typeof func == 'string') { func_n = func, func = JADR.国勢調査表示H27[func_n]; }
  const GEO_org = JADR.GEO.国勢調査;
  // (2023.01.18 sakamoto) 2年分の国勢調査結果を並べて表示が可能なパネルとなる。
  // 国勢調査比較が OFF の時および、横幅が 480px を下回る時に比較は出力しない。
  const myTurn = ()=>$g.is(GEO_org.$panel); // まだ次の tip が作成開始されていない場合
  Promise.resolve().then(()=>{

    // 過去分はあとからの表示でもいいので、非同期
    // TODO setTimeout にすることも検討
    const div = $(ECDB.HTML("#base"));
	const remove = ()=>{ 
	  $g.find('.cmn .hikaku').remove();
	  $("[name=地区名]",div).text('[ 平成27年/2015年 ]').siblings().remove();
	  $(div).append('<div><span>比較データはありません</span></div>');
	};
    if(!myTurn()) return;
    $g.find('.cmp').append(div);
    GEO_org.code['H27'] = { code, name, prop: d };
    GEO_org.data['H27'] = null;
    return JADR.GEO.国勢調査("00").then(t=>JADR.GEO.国勢調査(code.substr(0, 9)).then(val=>{ if (!val&&!name) return remove();
      GEO_org.data['H27'] = val;
      const areaName = code.length ==2 ? JADR.地域名称(code): code.length == 5 ? JADR.地域名称(code.substring(0,2))+JADR.地域名称(code): JADR.地域名称(code.substring(0,5))+name;
      if(!val||!func) return remove();
      $("[name=地区名]",div).text('[ 平成27年/2015年 ]');
      $("[name=総人口]",div).text(comma(val[1][0]));
      $("[name=世帯数]",div).text(comma(val[1][3]));
      $("[name=総面積]",div).html(comma((area*1e-6).toFixed(2)));
      $("[name=人口密度]",div).html(comma((val[1][0]/area*1e6).toFixed(1)));
      !val[1][0]?$("[name=コメント]",div).html("この区域には、世帯がありません"):
      !val[5][0]?$("[name=コメント]",div).html("この区域は、秘匿措置の対象区域です"):
      func(div, val, t, code); // 01_総務省 の関数を起動
      $("[name=領域]", div).text(areaName);
      $("[name=市区町村]", div).text(JADR.地域名称(code.substring(0,5)));
    })).then(()=>{
      const OLDv = JADR.GEO.国勢調査.data['H27'];
      const NEWv = JADR.GEO.国勢調査.data['R02'];
      if(!myTurn() || !OLDv || !NEWv) {
        return; // もう変更を受けている。
      }
      const $hikaku = $g.find('.cmn .hikaku');
      const w = $g.find('.cmn [name=地区名]').width() + 8;
      $hikaku.css('left', `calc(100% - ${Math.min(160, Math.floor(($g.width() - w) / 2))}px)`);
      // left:calc(100% - 160px);
      const calc = { 総人口: NEWv[1][0] - OLDv[1][0], 世帯数: NEWv[1][3] - OLDv[1][3] };
      const view = k=>{ 
        const n = calc[k]; 
        return n ? n > 0 ? `<span style="color:#0c0;">+${comma(n)}</span>`: 
          n < 0 ? `<span style="color:#fc6464;">${comma(n)}</span>`: '<span> 増減なし </span>': '<span> - </span>';
      };
      $('[name=総人口増減]', $hikaku).html(view('総人口'));
      $('[name=世帯数増減]', $hikaku).html(view('世帯数'));
    });

  })['catch'](e=>{
    console.error('比較表示:',e);
  });
}
function 印刷(code, func) {
  const div = $("<div/>").css({width:"100%", fontSize:13, color:"#000"});
  return JADR.GEO.国勢調査R2("00").then(t=>JADR.GEO.国勢調査R2(REPLACE(code.substr(0, 9)))
    .then(val=>val ? func(div, val, t, code, true).then(()=>$(".header", div).remove()).then(()=>div[0]):div[0]));
}
////=====================================================================================
function 国勢調査R2(flag) { var name = "国勢調査R2", PREFS = {}, hash = window._国勢調査R2_ = window._国勢調査R2_||{};
  ...
  function load(pref) { var a = [], b = [], c = [];
    ...
    const convert = {
    "T001081":t=>[ID(t[0]),clean(t.slice(7))],
    "T001082":t=>[ID(t[0]),clean(t.slice(28,43).concat([t[46]])), clean(t.slice(48,63).concat([t[66]]))],
    "T001083":t=>[ID(t[0]),clean(t.slice(7))],
    "T001084":t=>[ID(t[0]),clean(t.slice(7))],
    "T001085":t=>[ID(t[0]),clean(t.slice(7))],
    "T001086":t=>[ID(t[0]),clean(t.slice(7))],
    "T001103":t=>{
      // H27からフォーマット変更あり(「[29]総数」の追加)
      const a = clean(t.slice(7)); a.splice(29 - 7, 1);
      return [ID(t[0]),a];
    },
    "T001104":t=>[ID(t[0]),clean(t.slice(7))],
    "T001106":t=>[ID(t[0]),clean(t.slice(7))],
    };
...
////=====レポート用
/*
function 国勢調査R2(code) { let name = "国勢調査R2";
  let target = code.length > 5? code.substring(0,2): "00";
  code = +code;
  return hash[code]? Promise.resolve(hash[code]):
  JADR.loadTSV(name+"/"+target).then(putHash).then(()=>hash[code]);
  function putHash(v) { v.forEach(t=>hash[t[0]]=t); }
...
*/
////-----------------------------------------------------------------------------------------------
function 人口R2(div, val, japan, code, pflag) { 
  div.append(ECDB.HTML("[name=人口]"));
  const date = s => s.substring(0,4)+"-"+s.substring(4,6)+"-"+s.substring(6,8);
  $("[name=人口構成]",div).html(人口構成());
  if (pflag && code.length > 5) { $(".code",div).remove(); }
	eval('debugger;')
  return Promise.all([
    人口推移R2(code,pflag).then(v=>$("[name=人口推移]",div).html(v)),
    市区町村履歴(code).then(v=>{
...
////-----------------------------------------------------------------------------------------------
function 人口推移R2(flag, pflag) { var name = "人口推移R200"; var Q ={};
  // ここでは g-stat で提供されている Excel を解析する。
  // ファイルは変わらない。Excel に追記がされていく。
  //  => 違うかも。他統計との兼ね合いで令和2年分を表示していなかっただけかもしれない。アップロードして確認
  // だんだんシートが増えるので、ヘッダーを増やし、シート名を解析するロジックは年度ごとに確認が必要かもしれない。
  const num = s => +((""+s).replace(/,/g,""));
  var years = ["令和2年","平成27年","平成22年","平成17年","平成12年","平成7年","平成2年","昭和60年"] // "昭和60年"
  // urls は da0101 - da0647 までの 47 都道府県のファイルとなっている。
  // e.g.) https://www.e-stat.go.jp/stat-search/file-download?statInfId=000001085975&fileKind=0
  var urls = [
  ...
  return flag === true? thenEachPref(pref=>bucket.getXLSX(urls[(+pref)-1]).then(v=>{
    //  console.log(pref, v);
    //  years = シート名 [ ... 平成17年, 平成22年, 平成27年_原数値, 令和2年_原数値, 平成27年_不詳補完値, 令和2年_不詳補完値 ]
    years.forEach((year,i)=>{
      const vy = v[`${year}_不詳補完値`] || v[`${year}_原数値`] || v[year];
      // 前回(xls) vy.filter(t=>(""+t[0]).match(/^\d+$/))
      vy.filter(t=>(""+t[2]).match(/^(\d+)$/)).map(t=>{
        let code = REPLACE([ parseInt(t[2])<9999?"0":"", parseInt(t[2]) ].join(''));
        // 前回(xls) var code = REPLACE((t[0]<9999?"0":"")+t[0]);
        Q[code] = Q[code]||[];
        // フォーマット 男<15 15<=男<64 64<=男 女<15 15<=女<64 64<=女
        // 前回(xls) [ num(t[7]),num(t[8]),num(t[9]),num(t[11]),num(t[12]),num(t[13])　]
        Q[code][i] = [num(t[9]),num(t[10]),num(t[11]),num(t[13]),num(t[14]),num(t[15])];
      });
    });
  })).then(()=>{
  ...
  function SVG(a) { var size = a.length *10, y = size-10, fg = pflag?"#000":"#fff";
    ...
    ["1986","1991","1996","2001","2006","2011","2016","2021"].forEach((s,i)=>
...
////-----------------------------------------------------------------------------------------------
function 世帯R2(div, val, t, code, pflag) { let fg = pflag?"#000":"#fff";
  ...
  if (pflag && code.length > 5) { $(".code",div).remove(); }
  return 世帯収入R2(code.substring(0,5), pflag).then(v=>v? $("[name=世帯収入]",div).html(v): $("[name=世帯収入]",div).closest('.code').remove()).then(()=>div[0]);
  function 世帯人数(v) {
...
////-----------------------------------------------------------------------------------------------
function 世帯収入R2(flag, pflag) { var name = "世帯収入R200", q = {}, fg = pflag?"#000":"#fff";
  // ここでは g-stat で提供されている Excel を解析する。
  // 毎年公開される情報なので urls の追記が必要。
  const num = s => { s = (""+s).replace(/,/g,""); return isNaN(s)?0:+s;};
  const toCode = n => { n = Math.floor(n/100); return (n < 9999? "0":"")+n };
  // 元ネタ = 住宅・土地統計調査[00200522] 平成25年住宅・土地統計調査 / 確報集計 / 都道府県編（都道府県・市区町村） / 2013年
  // * 住宅・土地統計調査 は 2013 => 2018 => ... と 5年に 1回。2年後ぐらいに更新される。
  // 説明URL: https://www.e-stat.go.jp/statistics/00200522
  // [b89] 第38表　世帯の年間収入階級(9区分)，世帯の種類(2区分)，住宅の所有の関係(5区分)
  // 別普通世帯数，１世帯当たり人員，１世帯当たり居住室数及び１世帯当たり
  // 居住室の畳数
  // * 2013年データ利用中?
  // e.g.) https://www.e-stat.go.jp/stat-search/file-download?statInfId=000028111597&fileKind=0
  var urls = [
  ...
  // 世帯収入からのコピー方法
  // name = "世帯収入R200", b0 = name.substr(0, 4), b1 = name.substr(0, 6); 
  // when = Promise.resolve(); new Array(48).fill(0).map((z, i)=>i).forEach(i=>when = when.then(()=>{ const p = i == 0 ? '00': `/${i <= 9 ? '0': ''}${i}`; return Promise.resolve().then(()=>bucket.loadObject(`${b0}${p}`)).then(b=>bucket.saveObject(`${b1}${p}`, b)); })); when.then(()=>console.log('OK')['catch'](e=>console.error('NG', e)));
  return flag === true? thenEachPref(pref=> bucket.getXLS(urls[(+pref)-1]).then(v=>{

    // B089 は非常に複雑な構造をしている。
    // t[1]
    // 201　 松　　  江　　　市 <= t[1] から code が取れる理由
    // (その１.普   通   世  帯   数)
    // 普    通    世   帯   総   数 
    //    100  　万 　 円 　 未   満
    //    100         ～  　     200
    ...
    // 　1500  　万　  円　  以   上
    // t[5] 総　数
    // => 旧ロジックからみるに、昔は横ならびだったのかもしれない
    // => 一旦 世帯収入R200 は 世帯収入00 からコピー
  ...
  ((code)=>{ code = code.length === 2? code: code.substring(0,5);
    return JADR.loadObject(name).then(v=>{
      ...
      return v? SVG(v[1]):pflag?null:"<div>町村人口が15,000人以下のため<br/>統計がありません</div>";
    });
  })(flag);
...
function 住居R2(div, val, t, code, pflag) { let fg = pflag?"#000":"#fff";
  div.append(ECDB.HTML("[name=住居]"));
  $("[name=対象世帯数]",div).html(comma(val[6][0]));
  $("[name=住宅区分]",div).html(住宅区分(val[6]));
  $("[name=住宅建築]",div).html(住宅建築(val[7]));
  if (pflag && code.length > 5) { $(".code",div).remove(); }
  return 民賃情報R2(code.substring(0,5),pflag).then(v=>v? $("[name=民賃情報]",div).html(v): $("[name=民賃情報]",div).closest('.code').remove()).then(()=>div[0]);
  function 住宅区分(v) {
    var svg = svgHead(-5,-5,110,40);
    svgElem("rect",{x:0,y:0,width:100,height:15,fill:"#888"}).appendTo(svg)
...
////-----------------------------------------------------------------------------------------------
function 民賃情報R2(flag, pflag) { var name = "民賃情報R200",  q = {}, fg = pflag?"#000":"#fff";
  // ここでは g-stat で提供されている Excel を解析する。
  // 毎年公開される情報なので urls の追記が必要。
  const num = s => { s = (""+s).replace(/,/g,""); return isNaN(s)?0:+s;};
  const toCode = n => { n = Math.floor(n/100); return (n < 9999? "0":"")+n };
  ...
  // 民賃情報からのコピー方法
  // name = "民賃情報R200", b0 = name.substr(0, 4), b1 = name.substr(0, 6); 
  // when = Promise.resolve(); new Array(48).fill(0).map((z, i)=>i).forEach(i=>when = when.then(()=>{ const p = i == 0 ? '00': `/${i <= 9 ? '0': ''}${i}`; return Promise.resolve().then(()=>bucket.loadObject(`${b0}${p}`)).then(b=>bucket.saveObject(`${b1}${p}`, b)); })); when.then(()=>console.log('OK')['catch'](e=>console.error('NG', e)));
  return flag === true? thenEachPref(pref=> bucket.getXLS(urls[(+pref)-1]).then(v=>{

    //  console.log(pref, v);
    // => 一旦 民賃情報R200 は 民賃情報00 からコピー
...
////-----------------------------------------------------------------------------------------------
function 産業R2(div, val, t, code, pflag) { let fg = pflag?"#000":"#fff";
...
////-----------------------------------------------------------------------------------------------
function 職業R2(div, val, t, code, pflag) { let fg = pflag?"#000":"#fff";
...
////=====================================================================================
function 町丁目R2(flag, opts) { const name = 町丁目データ[0];
  return flag === true?
  thenEachPref(pref=>JADR.shape2geo(MAP_URL(pref)).then(geo=>write(geo, pref))):
  JADR.loadFeaturesBy(name, opts.xprefs);
  // 元ネタは国勢調査「国勢調査町丁・字等別境界データ」
  // 町丁目単位でその他国勢調査との整合を取るには該当調査年のデータを利用する必要がある。
  function write(geo, pref) { var names = {}, a = {};
    JADR.flatten(geo).forEach(t=>{
      ...
      names[id] = JADR.国勢調査関数.町丁目名変換(prop.S_NAME, true)
...
// ** 町丁目連動データ(1/2) ** //
function 地域名11桁R2(flag) { const name = 町丁目データ[1];
...
// ** 町丁目連動データ(2/2) ** //
function 地域名MAPR2(flag) { const name = 町丁目データ[2];
  return flag === true? readAll().then(write):JADR.loadObject(name);
  // 元ネタは「町丁目」
  function readAll() { var a = [];
    const fillNum = (n, len) => { n = "" + n; while (n.length < len) n = "0"+n; return n; };
...
function 市区町村履歴(code) { var name = "市区町村履歴";
  // 市区町村履歴からのコピー方法
  // name = "市区町村履歴R2", b0 = name.substr(0, 4), b1 = name.substr(0, 6); 
  // Promise.resolve(bucket.loadObject(`${b0}`)).then(b=>bucket.saveObject(`${b1}`, b)).then(()=>console.log('OK'))['catch'](e=>console.error('NG', e));
  return code === true? scripts("fr2HoldE").then(()=>JADR.saveTSV(name, window[name])):
  JADR.loadTSV(name).then(v=>v.filter(t=>t[1]==code).map(t=>[""+t[0],t[2].replace(/\+/g,"\n")]));
}
////-----------------------------------------------------------------------------------------------
// ** 使わない ** //
function 役所住所(code) { var name = "役所住所R2";
