...
async function build({ lang }) {
	...
	let open = true, spot = null;   // 起動中も開いている扱い＝閉じられたら片付ける
	const hide = () => { open = false; pane.classList.add("hidden"); spot?.clear(); spot = null; };
	...
	const map = await engine.default({
		...
		chips: false,              // 日本向けの層チップは出さない
		instruments: ["attr", "scale"],   // 出典（地理院/Natural Earth）は残す＝データの礼儀
	...
	const show = async (sign = {}) => {
		...
		spot = await map.gadget.spotlight(sign, { maxZoom: 6 });
