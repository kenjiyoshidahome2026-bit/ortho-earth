...
function frame() {
	...
	try {
		...
		if (dirty && renderer && cam) {
			...
			if (raster) raster.update(glCam, canvas.width, canvas.height, { groundR: groundRNow(glCam), moving: opts?.terrainGate === false || lastFrameRun - lastCamMoveT < RES_SETTLE_MS,
				cell: (terrain && !opts?.noTerrain && (glCam.pitch || 0) > 0.06) ? renderer.terrainCell?.() : null });   // moving＝入力中/飛行中/静定前＝取得の並列を絞る（着地で戻す）。cell＝地形メッシュの格子幅＝チルト時はタイル細分を地形より細かく（地形貫き防止）
			let fogAnim = false;
