...
// 実時間の枠（秒）を広げるページ＝段が多い実描画（t-request＝基図の差し替え・独自スキームの画像/3D Tiles/基図を順に）
const LONG_PAGES = { "t-request": 180 };
let rtSeq = 0;
...
async function runRealtime(url) {
	...
	try {
		...
		let title = "";
		const limitS = LONG_PAGES[url.match(/tests\/([\w-]+)\.html/)?.[1]] ?? 60;   // 長い実時間ページだけ枠を広げる
