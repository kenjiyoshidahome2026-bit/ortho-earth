...
const ALL_PAGES = ["t-gadgets", "t-newgadgets", "t-providers", "t-raster", "t-gndfaces", "t-measure", "t-profile", "t-shot", "t-palette-live", "t-demo", "t-scene", "t-print", "t-qr", "t-opts", "t-input", "t-narrow", "t-gintlod", "t-gintembed", "t-gintmultigl", "t-gintswap", "t-anno", "t-gintdepth", "t-rtl", "t-rtl?lang=ar", "t-model", "t-camera", "t-mllayers"];   // t-camera＝カメラの口（#35）
...
const REALTIME = new Set(["t-anno", "t-model", "t-raster", "t-gndfaces", "t-camera", "t-mllayers"]);   // t-camera＝easeTo/flyTo の Promise は rAF の実時間で着地（仮想時間では解決しない）   // t-gndfaces＝gint 面の地面アトラス焼き（標高の到着＝実時間）   // t-raster＝画像タイル層（MessagePort プロバイダ＝worker→worker のタイル・ラスタ PMTiles＝実時間の fetch）   // t-model＝model-worker 内の loaders.gl 動的 import（glb 直読み）
