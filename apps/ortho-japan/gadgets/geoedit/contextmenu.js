...
export function installContextMenu(ed) {   // 戻り値＝項目を搭載前（既定）へ戻す関数（本体地図に載った時の後片付け）
	...
	const cm = map.gadget.contextmenu({
		items: ctx => {
			...
			const popItem = { name: t("吹き出し(pop)を表示"), onClick: () => popLayer.open(under, { x: ctx.x, y: ctx.y, ll: ctx.lng != null ? [ctx.lng, ctx.lat] : undefined }) };   // 右クリック点を参照点に
			const selectItem = { name: t("この要素を選択"), onClick: () => { ed.setTool("select"); ed.select(under); } };
			const out = [];
			...
			out.push(
				...
				{ name: t("ここから線を描く"), onClick: c => { ed.setTool("line"); c.lng != null && ed.sketch.start("line", [c.lng, c.lat]); } },
				{ name: t("ここから面を描く"), onClick: c => { ed.setTool("polygon"); c.lng != null && ed.sketch.start("polygon", [c.lng, c.lat]); } },
			);
			if (m?.familyOf(m.feats.get(under)?.type || "") === "poly") out.push({ name: t("ここに穴を開ける"), onClick: c => { ed.setTool("hole"); c.lng != null && ed.sketch.start("hole", [c.lng, c.lat]); } });   // 穴はポリゴンの内側だけ
			out.push({ name: t("座標をコピー"), onClick: copyLL });
			return out;
		},
	});
