...
export function legend({ permanent, width } = {}) {
	...
	if (!permanent) {
		...
		close.className = "panel-close"; close.textContent = "×"; close.title = "閉じる"; close.setAttribute("aria-label", "凡例を閉じる");
		close.addEventListener("click", () => collapse());
		div.append(close);
		// 畳んだ時の再表示ボタン＝スタック（搭載順＝縦の並び）。内容がある間だけ意味を持つ。
		reopen = document.createElement("button");
		reopen.id = "legend-btn"; reopen.textContent = "凡例"; reopen.title = "凡例"; reopen.setAttribute("aria-label", "凡例を開く");
		reopen.dataset.tip = "凡例"; reopen.style.display = "none";
