...
- backend は `map.backend`、建物3D（日本＝PLATEAU）の読込合図は `map.on("mesh", e => e.phase)`（catalog→start→done。1.2.0〜・1.0.4〜1.1.x の旧名 "plateau" も別名で効く。1.0.3 以前はコンソール文字列＝llms.txt 罠⑯）。
