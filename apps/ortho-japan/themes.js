...
// 800（住居表示の町丁名）だけを一段奥へ送るノブ。800 は都市で密度が桁違い（実測 z16・市中心5km四方＝
// 京都2081件 / 札幌1330件 対 東京566件・高知205件。京都は「1ブロック1町」の粒度）。既定は 210 と同値＝
// 衝突間引き（labels2d）と丁目の低い symbol-sort-key に任せる。出過ぎるならここだけ上げる。
// ⚠実効の下限は16：800 は **z16タイルにしか無い**（実測・札幌/京都/東京の中心タイル＝z14:0件 z15:0件 z16:48件）。
// 210 は逆に広域用の抜粋で z14/15 が本場（z16では1件まで痩せる）＝二系統はズームでも役割が分かれている。
// なので z16 未満にこのノブを下げても何も変わらず、密度が跳ねるのは z16タイルへ切り替わる瞬間。
export const CHOME800_MINZOOM = CHOME_MINZOOM;
...
// 丁目：粒度が一段細かい→寄った時(z15.5〜)だけ自動表示。210「下落合（三）」と 800「下落合三丁目」は
// 同じ町丁の別表記＝どちらも地名（800 は住居表示の町丁名）。800 は CLAIMED 経由で施設バケツから外すのが要点で、
// 旧・残余バケツ落ち（施設＝他に属さない全部）では札幌1330件・京都2081件の町名が紫の施設名として出ていた。
// 表記の統一（→（N））と 210 との重複除去は main の mergeChome（app.js）が担う＝ここは分類だけ。
const CHOME_CODES = new Set([210, 800]);
...
// 8xx帯（住居表示・施設名の新系統 vt_flag17=2）にも自然地形が少数まぎれる＝コード直指定で拾う
// （実測：810＝山「愛宕山」・832＝坂「狸穴坂」「三宅坂」。同じ三宅坂が 332 にも居る＝二系統ある）。
const TERRAIN_EXTRA = new Set([810, 832]);
export const isTerrain = c => (c >= 300 && c <= 399) || TERRAIN_EXTRA.has(c);   // export＝main のラベル再スタイル（地形名＝濃い茶）でも使う
