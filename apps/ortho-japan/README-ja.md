...
> English: [README.md](README.md)（npm/配布物の正面はこちら・開発者向けの節=verify類はこの日本語版のみ）
...
容れ物は id・子要素・SDK が付けた class（ui-dark 等）まで元に戻る。視点を引き継ぐなら `view: map.view.hash` で再生成＝`lang`／`theme` を変える手段もこれ（live 切替 API は無い）。1.0.3 以前は WebGPU 起動後 20 秒以内の destroy で番犬タイマーが残りホストページごと reload した（1.0.4 で修正）。埋め込み（`target` 指定）ではページ URL を書き換えない（`urlHash: true` で本体サイトと同じ挙動）。地形の標高タイルは `plateau: false` でも `api.ortho-earth.com` から必要に応じて取得する（セッションで数十 MB）。
...
| `node scripts/bake-plateau.mjs` | **PLATEAU の R2 焼き**（全セットを Node で焼いて量子化 PLQ を `plateau-bake-out/` へ。`--upload`＝R2 へ put・要 `API_KEY`。`--shard=i/n` で並列・`--only=名前` で絞り込み・球/楕円体を同時に焼く・`--reverse` で別マシンと両端から挟む・`--redo-unbaked` で取れなかったタイルの仕上げ・`--repack` で焼き済みを PLQ2（溶接＋角柱）に詰め直す・`--stats` で角柱率の統計）。ブラウザは OPFS→R2 焼き→MLIT 生経路の順で引き、`?nobake=1` で封印・`?bake=URL` で置き場差し替え。`node tests/t-plateauq.mjs --real` が形式の往復検定 |
