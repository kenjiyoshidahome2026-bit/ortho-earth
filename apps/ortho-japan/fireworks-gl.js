...
//   星     … 球状（牡丹）に均一に飛び、速度は減衰（k=1/s）＝約 3 秒で開く直径に達する。重力で少し垂れ、寿命の終わりで消える
...
const G = 9.8, DAMP = 1.0, MAX_P = 32768;
...
export function starPos(v, t, k = DAMP) { const f = (1 - Math.exp(-k * t)) / k; return [v[0] * f, v[1] * f, v[2] * f - G * (t - f) / k]; }
...
void main() {
	...
	float a = v_col.a * (1.0 - d * d);   // 中心が明るい丸
...
let earthM = 6371000;
let shells = [];            // 上昇中の玉 { t0, v0, tApex, diam, stars, col, trailT }
let stars = [];             // 星 { t0, x, y, z, v:[3], life, col, size }
let sparks = [];            // 尾の火の粉 { t0, x, y, z, vz, life }
...
export function message(d) {
	...
	if (d.type === "site") { site = { lon: d.lon, lat: d.lat }; }
		...
		const col = COLORS[d.color != null ? d.color % COLORS.length : (Math.random() * COLORS.length) | 0];
		shells.push({ t0: d.now, v0, tApex, diam: sh.diam, stars: sh.stars, col, x: d.dx || 0, y: d.dy || 0, trailT: d.now });
		host?.requestDraw();
	} else if (d.type === "clear") { shells = []; stars = []; sparks = []; host?.requestDraw(); }
...
function step(now) {
	...
	for (const s of shells) {
		...
		if (t >= s.tApex) {   // 開く：星を球状に均一に
			const v = starSpeedOf(s.diam);
			for (let i = 0; i < s.stars; i++) {
				const u = 2 * rnd() - 1, ph = 2 * Math.PI * rnd(), rr = Math.sqrt(1 - u * u), sp = v * (0.9 + 0.2 * rnd());
				stars.push({ t0: s.t0 + s.tApex * 1000, x: s.x, y: s.y, z: s.apexZ ?? s.v0 * s.v0 / (2 * G), v: [rr * Math.cos(ph) * sp, rr * Math.sin(ph) * sp, u * sp], life: 2.6 + rnd() * 1.4, col: s.col, size: 1.2 + s.diam / 300 });
			}
			continue;
		}
		const z = s.v0 * t - 0.5 * G * t * t;
		put(s.x, s.y, z, 1.0, 0.85, 0.6, 0.9, 1.5);
		while (s.trailT < now) { s.trailT += 40; const tt = (s.trailT - s.t0) / 1000, zz = s.v0 * tt - 0.5 * G * tt * tt; sparks.push({ t0: s.trailT, x: s.x + (rnd() - 0.5) * 2, y: s.y + (rnd() - 0.5) * 2, z: zz, vz: -(rnd() * 8), life: 0.5 + rnd() * 0.4 }); }
		keep.push(s);
	}
	shells = keep;
	// 星
	...
	for (const st of stars) {
		const t = (now - st.t0) / 1000, k = t / st.life, p = starPos(st.v, t);
		const a = k < 0.7 ? 1 : 1 - (k - 0.7) / 0.3, tw = 0.75 + 0.25 * Math.sin(t * 23 + st.v[0]);   // 終わりに消える・少し瞬く
		put(st.x + p[0], st.y + p[1], st.z + p[2], st.col[0], st.col[1], st.col[2], a * tw, st.size * (k < 0.3 ? 1.4 : 1.0));
	}
	// 尾の火の粉
	sparks = sparks.filter(sp => (now - sp.t0) / 1000 < sp.life);
	for (const sp of sparks) { const t = (now - sp.t0) / 1000; put(sp.x, sp.y, sp.z + sp.vz * t, 1.0, 0.6, 0.2, 0.7 * (1 - t / sp.life), 0.6); }
	return n;
}
...
export const stats = () => ({ particles: statN, shells: shells.length, stars: stars.length, t: lastT, shellT: shells.map(sh => +((lastT - sh.t0) / 1000).toFixed(2)) });   // shellT＝各玉の経過秒（負＝時計がまだ打ち上げ前）
...
export function destroy() {
	...
	gl = null; host = null; shells = []; stars = []; sparks = [];
}
