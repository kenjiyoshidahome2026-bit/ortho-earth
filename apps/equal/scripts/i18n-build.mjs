#!/usr/bin/env node
// 訳の正本 i18n/ui.json（equal 固有の語）から、実行時が読む薄い表 i18n/lang/<code>.json を焼く。
// 共有の語（Layers & themes・Roads・Opacity…）は ortho-japan の辞書を実行時に重ねる＝ここには持たない。
// 作法は ortho-japan の scripts/i18n-build.mjs と同じ（空文字・欠落は落とす＝実行時は「表に無い＝英語」の一本道）。
