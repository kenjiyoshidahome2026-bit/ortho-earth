...
// opts:
//   value: (item, index) => number | string | null
//   type: "quantile" | "equal" | "categorical" | "political"（political＝値そのものを 1..13 の色番号として使う）
//   classes: 分類数（連続量）・ramp: RAMPS のキー or 色配列・colors: 質的の色配列
//   format: 凡例の数値書式
export function buildChoropleth(items, opts) {
	const n = items.length, rgba = new Uint8Array(n * 4), values = new Array(n);
	for (let i = 0; i < n; i++) { const v = opts.value(items[i] || {}, i); values[i] = v; }
	const legend = [];
	...
	if (opts.type === "equal") {
		...
		breaks = Array.from({ length: k - 1 }, (_, j) => lo + (hi - lo) * (j + 1) / k);
	} else {
